...
function Top3Inner() {
  ...
  function handleSelectSuggestion(s: Suggestion) {
    setTitle(s.title)
    if (s.imageUrl) setPickImage(s.imageUrl)
  ...
  async function handleSend() {
    ...
    try {
      ...
      await addPick(userId, finalCat, title.trim(), why.trim() || undefined, links.filter(l => l.trim()), city.trim() || undefined, pickImage || undefined)
      setSending(false)
      setSent(true)
    } catch {
      setSending(false)
      ...
      <div className="flex-1 overflow-y-auto scrollbar-none px-4 pt-4 pb-6">
        <div className="bg-bg-card border border-border rounded-card px-4 py-4">
          ...
          <CategoryChips
            category={category}
            customCat={customCat}
            onCategoryChange={(cat) => { setCategory(cat); setTitle('') }}
                ...
                <div className="anim-in mb-4">
                  {pickImage ? (
                    <div className="relative rounded-xl overflow-hidden" style={{ height: 160 }}>
                      <img src={pickImage} alt="" className="w-full h-full object-cover" />
